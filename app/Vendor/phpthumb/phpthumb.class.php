		...
		if ($commandline) {
			if ($IMtempfilename = $this->phpThumb_tempnam()) {
				...
				if ($this->ImageMagickSwitchAvailable('thumbnail') && $this->config_imagemagick_use_thumbnail) {
					...
					$commandline_test = $this->ImageMagickCommandlineBase().' logos: -resize 1x "'.$IMtempfilename.'" 2>&1';
					...
					if ($fp_im_temp = @fopen($IMtempfilename, 'wb')) {
						// erase temp image so ImageMagick logos doesn't get output if other processing fails
						fclose($fp_im_temp);
					}
				}
